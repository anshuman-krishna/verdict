...
# bundled into the extension at build time." This module is that file's
# format, on both ends: the pipeline writes it here and the extension reads
# the same keys in src/score/model.ts.
#
# The absent form matters as much as the present one. extension/src/score
# has to import something at build time whether or not a model exists yet,
# and an empty object or a zeroed model would both read downstream as a
# real model predicting no risk. So absence is stated, not implied.
...
# sorted keys and a trailing newline so re-exporting an unchanged model
# produces no diff, the same reason the canary status document is written
# this way.

...
// PLAN.md week 1 task 6. Two modes, because "is extraction regressing" and
// "does version 0.1 meet SPEC.md section 14" are different questions with
// different right answers today. The per commit run fails only on a fixture
// that broke without anyone documenting why. The gated run, behind
// `just fixtures`, additionally holds the whole corpus to section 14 and
// fails when it is empty, since a corpus nobody has built yet has measured

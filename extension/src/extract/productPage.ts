// SPEC.md section 9's four locales. The host suffix is the only thing that varies enough to be
// worth a lookup table; the rest of amazon's product url shape (/dp/{ASIN} and /gp/product/{ASIN})
// is stable across all of them and has been for years, unlike anything selector based.
...
// null for anything that is not a recognised amazon product page: a different host, a search
// results page, a cart, a seller storefront. SPEC.md section 13: "page is not a product page:
// render nothing at all", and this is the function that decision starts from.
...
// SPEC.md section 9's "fetching additional pages" for a given page number, in the same shape
// amazon's own pagination links use. Not itself a fetch, callers decide when and whether to use it.

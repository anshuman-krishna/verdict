...
// that corpus is PLAN.md week 4, not built yet. the label corpus and its methodology are anshuman's
// (SPEC.md section 12), so nothing here invents real per category numbers.
//
// PLACEHOLDER_PRIORS exists only so the pipeline can run end to end before that corpus exists.
// organicPrior is flat (maximally uninformative: every star bin equally likely), so injectedShare
// below measures deviation toward the injection kernel and nothing else. injectionKernel follows

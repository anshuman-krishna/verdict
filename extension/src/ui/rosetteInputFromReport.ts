...
// panel.ts takes a Report and a RosetteInput as two separate arguments, since bootstrap.ts and
// buildReport.ts do not carry burstShare and duplicateShare on Report itself. This is the one place
// that reunites them, by name, off evidence.ts's fixed row set, so a caller mounting the panel from
// a ReportOutcome never has to know evidence.ts's row order.

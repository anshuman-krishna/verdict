...
// SPEC.md section 16 lists "history default on/off" as an open question for anshuman. true is a
// placeholder starting value so the options page toggle has something to render before that
// decision is made; it is not itself the decision.
...
// SPEC.md section 4: the reviewer graph service is "opt in, off by default". Unlike
// DEFAULT_HISTORY_ENABLED above, this default is not a placeholder standing in for an undecided
// value, it is what the spec already says.
const DEFAULT_REPUTATION_LOOKUP_ENABLED = false;

// deliberately not a synced pref, unlike historyEnabled above: syncedBoolean.ts's own comment
// explains why a permission gated toggle is not "where it makes sense" for chrome.storage.sync.
...
// PRIVACY.md section 5: "opt in contribution", off by default, a separate switch from
// reputationLookupEnabled above. Asking whether a reviewer is flagged (section 4) and contributing
// edges that help build the graph in the first place (section 5) are two different consents:

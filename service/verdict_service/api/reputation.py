...
# SPEC.md section 8, the k anonymous reputation lookup protocol. the extension computes
# sha256(reviewer_id + salt) locally, sends only the first PREFIX_LENGTH hex characters of each
# hash, deduplicated and padded with random prefixes to exactly BUCKET_COUNT, and matches the
# response against its own hashes locally. this endpoint never receives a reviewer id, a product
# identifier, or a real hash, only prefixes, and it never has any way to tell which of the
# BUCKET_COUNT prefixes in a given request were real.
...
# a plain function, not a class with request state, because this handler reads nothing from the
# request except the validated body: no cookie, no header, no client address. that is enforced by
# never referencing `request` at all, not by a filter that could be forgotten later.

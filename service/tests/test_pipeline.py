...
def test_compute_flagged_hashes_end_to_end():
    # a pair, not a larger clique: backbone.py's disparity filter only keeps an edge that dominates
    # one of its endpoints' own weight distribution, and a node with exactly one significant edge
    # always qualifies (backbone.py's degree <= 1 special case). A larger clique of perfectly
    # symmetric overlaps, by contrast, has every edge tied for that node's whole distribution and
    # none of them stands out, so the filter (correctly) drops all of them; that is real behaviour
    # of the algorithm, not a gap in this pipeline.
    tight_group = ["f1", "f2"]
    reviewer_products = {
        ...
        **{reviewer: {"p1", "p2", "p3"} for reviewer in tight_group},
        # unrelated, single product reviewers: never share anything with anyone, so they can never
        # form a significant edge, appear in no community, and must never be flagged.
        "g1": {"p4"},
        "g2": {"p6"},
...
class TestComputeFlaggedHashesFromContributions:
    # api/contribution.py's real input shape: one ContributionEdge per review, already pseudonymised
    # client side, never a raw reviewer id or a reviewer_products dict handed in pre-aggregated.
    ...
    def test_end_to_end_reuses_the_same_hand_checked_scenario_as_compute_flagged_hashes(self):
        ...
        # the community's members ARE the flagged hashes: no re-hashing, since a ContributionEdge's
        # reviewer_hash already is what a reputation lookup query would compute.
    ...
    def test_converts_week_bucket_to_an_approximate_day_index_rather_than_treating_it_as_days(self):
        # two reviewers a week apart (week_bucket differs by 1) should read as tightly clustered
        # against the 365 day reference span; if week_bucket were mistakenly treated as a day count
        # directly, the temporal spread would still round to "tight" here too, so this specifically
        # checks a gap large enough to only make sense in weeks: 52 weeks apart is a full year in
        # real time, but only 52 units apart if misread as days.
        edges = [
        ...
        # same ratings, so only temporal clustering can be what keeps this under the flag threshold:
        # 52 weeks converted to days (364) reads as almost the entire reference span, correctly
        # reading as spread out rather than tight.
        assert compute_flagged_hashes_from_contributions(edges) == set()
